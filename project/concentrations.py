...
def concentrations(writer, basic_data, pk_concs, column_names):
    ...
    for i in range(int(len(basic_data["group"]) / 2)):
        ...
        animal_conc = grouped.pivot(
            ...
            values=column_names['measured'][0],
        )
        ...
        for group in basic_data["group"][i * 2 : i * 2 + 2]:
            ...
            for animal in animals:
                ...
                for day in basic_data["day"]:
                    ...
                    X = for_plot["Time"].replace({"h": ""}, regex=True).astype(float)
                    Y = for_plot[column_names['measured'][0]]
            ...
            # Plot means
            X = animal_mean.index.get_level_values(2).to_series().replace({"h": ""}, regex=True).astype(float).drop_duplicates()
            for day in basic_data["day"]:
                Y = animal_mean.loc[(day)]
                plt.plot(X, Y, label=f"{group} mean - {day}", marker=".")
            plt.legend()
            plt.title(f'{group} mean')
            plt.xlabel("Time [h]")
            plt.xlim(0, 24)
            plt.xticks(np.arange(0, 25, step=2))
            plt.ylabel("Conc. [ng/mL]")
...
def add_cols(df, data, column_names):
    """
    ...
    df[column_names['measured'][0]] = (df[column_names['measured'][0]].replace({"No Peak": "0"}).astype("float64"))
    return df
